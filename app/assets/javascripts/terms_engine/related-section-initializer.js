$(function() {
  ...
  $('#details-tab-link[data-toggle="tab"]').on('shown.bs.tab', function (e) {
    //Code for Summary
    if(!summaryLoaded){
      $("#relation_details .tab-content-loading").show();
      relatedSolrUtils.getTermsSummaryElements().then(function(result){
        var feature_label = $($('#related_js_data').data('featureLabel'))[0].innerText;
        var feature_path = $('#related_js_data').data('featurePath');
        relatedSolrUtils.addTermsSummaryItems(feature_label,feature_path,'parent',result);
        relatedSolrUtils.addTermsSummaryItems(feature_label,feature_path,'child',result);
        relatedSolrUtils.addTermsSummaryItems(feature_label,feature_path,'other',result);
        ...
        if(!columnizedApplied){
          ...
          $('.kmaps-list-columns:not(.subjects-in-subjects):not(.already_columnized)').addClass('already_columnized').columnize({
            width: 330,
